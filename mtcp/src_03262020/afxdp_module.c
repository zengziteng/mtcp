...
afxdp_init_handle(struct mtcp_thread_context *ctxt)
{
	...
	for (j = 0; j < num_devices_attached; j++) {
		...
		axpc->cfg.ifindex = 3;//devices_attached[j]; //(enp0s8) m-> 3 if not getting correctly from mTCP
...
void afxdp_release_rx_ring(struct mtcp_thread_context *ctxt){
	struct afxdp_private_context *axpc;
	//printf("afxdp_release_rx_ring\n");

	axpc = (struct afxdp_private_context *)ctxt->io_private_context;
	xsk_ring_cons__release(&axpc->xsk_socket->rx, axpc->rcvd);
